...
for line in model:
    ...
    if s[0] == 'v':
        ...
        f = [tuple(part.split('/')) for part in s[1:4]]
        F.append([int(f[0][0]), int(f[1][0]), int(f[2][0])])
        FT.append([int(f[0][1]), int(f[1][1]), int(f[2][1])])
...
print("Шаг 5: Нормали к вершинам вычислены")
...
print("Шаг 6: Рендеринг треугольников...")
for idx, (face, tex) in enumerate(zip(F, FT)):
    if idx % 500 == 0:
        print(f"   -> Обработка полигона {idx}/{len(F)}")
...
img.save('img.png')
print("Шаг 7: Сохранение изображения завершено.")
